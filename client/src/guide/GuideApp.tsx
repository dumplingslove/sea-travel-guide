...
function Detail({item,kind,onClose}:{item:Item;kind:'酒店'|'餐厅'|'景点';onClose:()=>void}){
 ...
 const facts=getGuideFacts(item,kind); const evidence=xhsEvidence[item.name]||[]; const gallery=getPlaceGallery(kind,item); const image=gallery[0]?.src; const a=getXhsAssessment(item); const operationalNotice=operationalNotices[item.name];
...
function RestaurantCatalog(){
 ...
 return <div className="page"><PageHero eyebrow="DINING PLAN" title="餐厅预订决策指南" summary="把每日吃什么、必须提前订的餐桌、街头小吃与风险提醒放进同一套决策流程。" image={bangkokImg}/><div className="viewtoggle" role="tablist"><button className={view==='每日用餐计划'?'active':''} onClick={()=>setView('每日用餐计划')}>每日用餐计划</button><button className={view==='按城市浏览'?'active':''} onClick={()=>setView('按城市浏览')}>按城市浏览</button></div>{view==='每日用餐计划'?<section className="sectionblock mealplan"><div className="sectiontitle"><h2>20 天用餐路线</h2><p>按行程日查看正餐、备选与不要硬塞的组合</p></div>{route.map(([c])=><section key={c}><header><img src={cityImages[c]} alt=""/><div><span>{route.find(x=>x[0]===c)?.[2]}</span><h3>{c}</h3></div></header><div>{days.filter(d=>d.city===c).map(d=><article key={d.day}><b>Day {d.day}</b><div><h4>{d.title}</h4><p>{d.food}</p></div><button onClick={()=>{setCity(c);setView('按城市浏览')}}>查看 {c} 餐厅</button></article>)}</div></section>)}</section>:<section className="sectionblock"><div className="sectiontitle"><h2>按城市浏览</h2><p>{restaurants.length} 个餐饮选择 · 星级餐厅、老店与街头小吃分开判断</p></div><CityTabs city={city} setCity={setCity} label="餐厅城市筛选"/><div className="bookingbrief"><strong>{city} · {cityDays.length}天</strong><p>{cityDays.map(d=>d.food).join(' ')}</p></div><div className="catalog rich">{shown.map((it,i)=><article className="itemcard" key={it.name}><ItemMedia item={it} kind="餐厅" index={i}/><div><span className="citytag">{it.city} · 餐饮</span><h3>{it.name}</h3><p className="meta">{it.meta}</p><p>{it.detail}</p><div className="decision"><b>订位 / 点单</b><span>{it.best}</span></div><XhsMini item={it}/><div className="cardactions"><button className="solid" onClick={()=>setChosen(it)}>查看完整攻略</button><a href={mapLink(it.name,it.city)} target="_blank" rel="noreferrer">地图 App</a></div></div></article>)}</div></section>}<ResearchStatus/>{chosen&&<Detail item={chosen} kind="餐厅" onClose={()=>setChosen(null)}/>}</div>
}
...
function ItineraryHotelList({city,entries,onOpen}:{city:string;entries:ResearchHotelEntry[];onOpen:(match:CatalogMatch)=>void}){
 ...
 return <section className="itinerary-hotels" aria-label={`${city}酒店参考`}><header><span>STAY OPTIONS</span><h3>本城酒店参考</h3></header><div>{options.map(item=>{const chain=findHotelChain(entries,item.name,item.city);const tier=findHotelTier(entries,item.name,item.city);return <button key={item.name} onClick={()=>onOpen({item,kind:'酒店'})} aria-label={`查看${item.name}酒店攻略`}><span><b>{item.name}</b><small>{item.meta}</small></span><HotelBadges chain={chain} tier={tier}/></button>})}</div></section>
}
...
function Flights(){return <div className="page"><PageHero eyebrow="FLIGHT PLAN" title="航班信息" summary="九段移动按单向路线排布。未确认航班号、实时票价与库存不会被写成事实。" image={bangkokImg}/><section className="sectionblock"><div className="sectiontitle"><h2>航段总览</h2><p>出票后把航班号、时间与确认号存进“我的预订”</p></div><div className="flightsummary"><article><b>9</b><span>个航段</span></article><article><b>2</b><span>预计经停 / 中转</span></article><article><b>4</b><span>入境国家</span></article></div><div className="flightgrid">{legs.map((l,i)=><article key={l[0]}><header><span>FLIGHT {String(i+1).padStart(2,'0')}</span><em>待预订</em></header><h3>{l[0]}</h3><strong>{l[1]}</strong><p>{l[2]}</p><dl><div><dt>出票前</dt><dd>核验日期与机场</dd></div><div><dt>出票后</dt><dd>保存航班号与确认号</dd></div></dl></article>)}</div></section><section className="checklistband"><h2>每段都要核对</h2><div><span>航站楼</span><span>托运行李额</span><span>转机签证</span><span>最短衔接时间</span><span>末班接驳</span><span>取消与改签</span></div></section><Source>固定研究快照 · 2026-09-12；不含实时航班数据</Source></div>}
...
function Transport(){return <div className="page"><PageHero eyebrow="GROUND PLAN" title="交通指南" summary="跨城航段与八城落地方式分开看：先锁定主线，再为每个城市选择最低摩擦的移动组合。" image={klImg}/><section className="sectionblock"><div className="sectiontitle"><h2>跨城衔接</h2><p>所有时间均为行程规划参考，不代替出票信息</p></div><div className="transportlist">{legs.slice(1,-1).map((l,i)=><article key={l[0]}><span>{String(i+1).padStart(2,'0')}</span><div><h3>{l[0]}</h3><p>{l[1]}</p></div><b>{l[2]}</b></article>)}</div></section><section className="mobility"><div className="sectiontitle"><span>ON THE GROUND</span><h2>八城落地策略</h2></div><div>{cities.map(c=><article key={c}><header style={{background:accents[c]}}><b>{c}</b><span>{cityMobility[c]?.[0]}</span></header><p>{cityMobility[c]?.[1]}</p><small>{cityMobility[c]?.[2]}</small></article>)}</div></section><section className="darkpanel"><h2>交通底线</h2><div><article><h3>海岛出行</h3><p>船班受海况影响，贵重物品放防水袋，不把次日早班机排得过紧。</p></article><article><h3>机场衔接</h3><p>跨国航班与托运行李需要更长缓冲；出票后再固化当天时间线。</p></article><article><h3>网约车</h3><p>按 App 显示车牌核对，机场与码头上车点提前确认。</p></article><article><h3>步行日</h3><p>热带正午把室外长距离拆开，以室内馆、咖啡或酒店休息降温。</p></article></div></section></div>}
