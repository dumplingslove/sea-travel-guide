...
/** 攻略信息图卡片网格（跨城分组复用） */
function IgGrid({items,offset,setOpen}:{items:Infographic[];offset:number;setOpen:(i:number)=>void}){return <div className="iggrid">{items.map((g,i)=><article key={g.file} className="igcard"><button onClick={()=>setOpen(offset+i)} aria-label={`放大查看${g.title}`}><img src={g.file} alt={g.title} loading="lazy"/></button><div><b>{g.title}</b><p>{g.summary}</p><small>图片来源：{g.source}{g.postUrl&&<><span> · </span><a href={g.postUrl} target="_blank" rel="noreferrer">原帖 ↗</a></>}</small></div></article>)}</div>}

/** 攻略信息图板块：本城归档图 + 末尾"跨城参考"分组 */
function InfographicPanel({city}:{city:string}){const cityItems=getInfographics(city);const crossItems=getCrossCityInfographics();const items=[...cityItems,...crossItems];const [open,setOpen]=useState<number|null>(null);if(!items.length)return null;return <><section className="sectionblock igsection" aria-label={`${city}攻略信息图`}><div className="sectiontitle"><span>XHS STRATEGY INFOGRAPHICS</span><h2>攻略信息图</h2><p>{cityItems.length?`${city}小红书攻略帖中的行程路线与实用信息图`:'跨城通用的行程参考信息图'}，点击放大查看。图中的门票、时间、交通信息仅供行前参考，出行前请向官方渠道复核。</p></div>{cityItems.length>0&&<IgGrid items={cityItems} offset={0} setOpen={setOpen}/>}<div className="iggroup"><h3>跨城参考</h3><p>新马泰多城通用的花费明细、住宿明细、常用 APP、路线交通图与行李清单。</p></div><IgGrid items={crossItems} offset={cityItems.length} setOpen={setOpen}/></section>{open!==null&&<InfographicViewer items={items} index={open} setIndex={setOpen} onClose={()=>setOpen(null)}/>}</>}
